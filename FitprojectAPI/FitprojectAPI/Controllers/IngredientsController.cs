...
namespace FitprojectAPI.Controllers
{
    ...
    public class IngredientsController : ControllerBase
    {
        ...
        [HttpPost("newIngredients")]
        public IActionResult PostR(FitprojectIngredient newingredient)
        {
            using (var context = new FitprojectContext())
            {
                try
                {
                    context.FitprojectIngredients.Add(newingredient);
                    context.SaveChanges();
                    return Ok($"Sikeres hozzáadás: {newingredient}");
                }
                catch (Exception ex)
                {
                    return BadRequest(ex.Message);
